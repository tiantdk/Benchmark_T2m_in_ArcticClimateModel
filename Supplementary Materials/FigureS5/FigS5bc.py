...
# -*- coding: utf-8 -*-
"""
Created on Thu Mar 2 10:42 2023
...
for ff in files:
    ...
    if ff == 'cmip6': eyear = 2014 #
    years = np.arange(syear, eyear+1, 1)
    
    # calculate the mean bias
    #ana_bias = cdo.sub(input = "-selyear," +str(syear)+"/"+str(eyear)+" " + files[ff]+" -selvar,t2m -selyear,"+str(syear)+"/"+str(eyear)+" OBS_T2M_ARCTIC_1deg_1982-2020_m15pct.nc")
    ...
    file_npz = 'FigS5_bias_map_'+ ff + '_' +str(syear) + '-' + str(eyear) + '.npz'
...
plt.savefig('FigS5_bias_maps_'+ str(syear) + '-' + str(eyear) + '.png',dpi=200,bbox_inches='tight')
